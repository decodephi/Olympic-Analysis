...
df = pd.read_csv('athlete_clean_events.csv')
...
if user_menu == 'Athlete wise Analysis':
    ...
    # Ensure x and name are aligned and exclude empty lists
    filtered_x = []
    filtered_name = []

    for data, label in zip(x, name):
       if len(data) > 0:  # Only include if list is not empty
        filtered_x.append(data)
        filtered_name.append(label)

# Now use the filtered version
    if filtered_x:
      ...
      st.warning("No data available to display distribution plot.")
